...
var LEFT_PADDING, RIGHT_PADDING, EXTENSION_PLACE, EXTENSION_INDEX, SHOW_SECONDS, gschema, lastExtensionPlace, lastExtensionIndex;
var settings, onLeftPaddingChanged, onRightPaddingChanged, onExtensionPlaceChanged, onExtensionIndexChanged;
let _httpSession;
let ccMenu;
...
const ContestCountdown = new Lang.Class({
    ...
    _init: function (settings) {
        ...
        this.buttonText = new St.Label({
            text: _("Loading..."),
            style: "padding-left: " + this.settings.get_int("left-padding") + "px;" + "padding-right: " + this.settings.get_int("right-padding") + "px; ",
            y_align: Clutter.ActorAlign.CENTER,
            x_align: Clutter.ActorAlign.FILL,
        });

        // Listen for update of padding in settings
        onLeftPaddingChanged = this.settings.connect("changed::left-padding", this._leftPaddingChanged.bind(this));
        onRightPaddingChanged = this.settings.connect("changed::right-padding", this._rightPaddingChanged.bind(this));
    ...
    _leftPaddingChanged: function () {
        this.buttonText.set_style("padding-left: " + this.settings.get_int("left-padding") + "px; " + "padding-right: " + this.settings.get_int("right-padding") + "px; ");
    },
    ...
    _refreshTimer: function () {
        ...
        // set new timeout
        this._timeout = Mainloop.timeout_add_seconds(1, Lang.bind(this, this._refreshTimer));
        return true;
    },
    ...
    _getTimerText: function () {
        ...
        else if (timeDiff == this.settings.get_int("notify-before")) Main.notify("Contest Countdown", this.contests.nextContest.name);
        ...
        if (this.settings.get_boolean("show-seconds")) return `${dd}d  ${hh}h  ${mm}m  ${ss}s`;
...
function enable() {
    // Load schema
    gschema = Gio.SettingsSchemaSource.new_from_directory(Self.dir.get_child("schemas").get_path(), Gio.SettingsSchemaSource.get_default(), false);
    ...
    onExtensionPlaceChanged = this.settings.connect("changed::extension-place", this.onExtensionLocationChanged.bind(this));

    onExtensionIndexChanged = this.settings.connect("changed::extension-index", this.onExtensionLocationChanged.bind(this));

    ccMenu = new ContestCountdown(settings);
    Main.panel.addToStatusArea("cc-indicator", ccMenu, settings.get_int("extension-index"), settings.get_string("extension-place"));
}
...
function onExtensionLocationChanged(settings, key) {
    ...
    if (this.lastExtensionPlace !== newExtensionPlace || this.lastExtensionIndex !== newExtensionIndex) {
        ...
        switch (newExtensionPlace) {
            case "left":
                Main.panel._leftBox.insert_child_at_index(ccMenu.container, newExtensionIndex);
                break;
            case "center":
                Main.panel._centerBox.insert_child_at_index(ccMenu.container, newExtensionIndex);
                break;
            default:
                Main.panel._rightBox.insert_child_at_index(ccMenu.container, newExtensionIndex);
        }
    }
}
